...
async function generateModels() {
	...
	// Add GLM models for z.ai provider not yet in models.dev catalog
	const zaiMissingModels: Model<"openai-completions">[] = [
		{
			id: "glm-5.1",
			name: "GLM-5.1",
			api: "openai-completions",
		...
		},
		{
			...
			reasoning: true,
			input: ["text"],
			cost: { input: 1.2, output: 5, cacheRead: 0.3, cacheWrite: 0 },
			compat: { supportsDeveloperRole: false, thinkingFormat: "zai", zaiToolStream: true },
			contextWindow: 204800,
			maxTokens: 131072,
		},
		{
			...
			reasoning: true,
			input: ["text"],
			cost: { input: 2.2, output: 8.9, cacheRead: 0.45, cacheWrite: 0 },
			compat: { supportsDeveloperRole: false, thinkingFormat: "zai", zaiToolStream: true },
		...
		},
		{
			...
			reasoning: true,
			input: ["text"],
			cost: { input: 1.1, output: 4.5, cacheRead: 0.22, cacheWrite: 0 },
			compat: { supportsDeveloperRole: false, thinkingFormat: "zai", zaiToolStream: true },
			contextWindow: 131072,
			maxTokens: 98304,
		},
	...
	for (const model of zaiMissingModels) {
		if (!allModels.some(m => m.provider === "zai" && m.id === model.id)) {
